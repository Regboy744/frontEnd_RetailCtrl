...
export function useOrderSubmission() {
  ...
  */
 function isSupplierSelectedForProduct(
  productId: string,
  supplierId: string,
 ): boolean {
 ...
 function toggleSelection(
  ...
  if (existing?.supplier_id === supplier.id) {
   ...
   const selection: ProductSelection = {
    ...
    savings:
     (product.order.unit_cost - price.unit_price) * product.order.quantity,
 ...
 function selectAllBestPrices(
  ...
  for (const product of products) {
   ...
   if (
    ...
    product.evaluation.winning_supplier_id
   ) {
    const supplier = suppliers.find(
     (s) => s.id === product.evaluation.winning_supplier_id,
    )
    const prices =
     product.supplier_prices[product.evaluation.winning_supplier_id]
