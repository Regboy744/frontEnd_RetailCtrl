...
interface Props {
 open: boolean
 supplierTotal: SupplierTotal | null
...
const sheetTitle = computed(() => {
 if (!props.supplierTotal) return 'Product Details'
 if (props.isLocalOrder) {
  return 'Products Where Order is Best'
 }
 return `Products Where ${props.supplierTotal.supplier_name} is Cheaper`
})

// Get the description
const sheetDescription = computed(() => {
 const count = props.products.length
 if (!props.supplierTotal) return ''
 if (props.isLocalOrder) {
  return `${count} product${count !== 1 ? 's' : ''} where your current order price is the best option`
 }
 return `${count} product${count !== 1 ? 's' : ''} where switching to ${props.supplierTotal.supplier_name} would save money`
})

// Get supplier price for a product
const getSupplierPrice = (product: ProductComparison): number | null => {
 if (!props.supplierTotal || props.isLocalOrder) return null
 const prices = product.prices[props.supplierTotal.supplier_id]
...
const getSupplierCode = (product: ProductComparison): string => {
 if (!props.supplierTotal || props.isLocalOrder) return '-'
 const prices = product.prices[props.supplierTotal.supplier_id]
