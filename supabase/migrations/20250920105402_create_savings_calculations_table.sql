-- 12. Savings Calculations (depends on: order_items, suppliers, orders, companies)
DROP TABLE IF EXISTS savings_calculations CASCADE;
CREATE TABLE savings_calculations (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    company_id UUID NOT NULL REFERENCES companies(id),
    order_item_id UUID NOT NULL REFERENCES order_items(id) ON DELETE CASCADE,
    baseline_order_id UUID NOT NULL REFERENCES orders(id) ON DELETE CASCADE,
    baseline_supplier_id UUID NOT NULL REFERENCES suppliers(id),
...
COMMENT ON COLUMN savings_calculations.baseline_order_id IS 'Reference to the internal baseline order used for this calculation';
COMMENT ON COLUMN savings_calculations.baseline_supplier_id IS 'The internal supplier providing the baseline price';
COMMENT ON COLUMN savings_calculations.baseline_price IS 'The baseline (internal) price for comparison';
...
COMMENT ON COLUMN savings_calculations.best_external_supplier_id IS 'The external supplier with the best price (excluding internal)';
