...
BEGIN
  ...
  GROUP BY s.id, s.name, s.is_internal;
END;
$$ LANGUAGE plpgsql STABLE;
...
BEGIN
  ...
  SELECT 
    ...
    up.full_name,
    up.email,
    COALESCE(SUM(sc.savings_amount), 0)::NUMERIC,
    COALESCE(AVG(sc.savings_percentage), 0)::NUMERIC,
    COUNT(DISTINCT o.id),
...
-- ============================================
-- NEW FUNCTION: Get baseline order details with savings
-- ============================================
CREATE OR REPLACE FUNCTION get_baseline_order_savings(
  p_baseline_order_id UUID
)
RETURNS TABLE (
  baseline_order_id UUID,
...
BEGIN
  ...
  SELECT 
    bo.id,
    bo.baseline_file_reference,
    bo.baseline_upload_date,
    oi.id,
    mp.description,
    sc.baseline_price,
    sc.chosen_price,
  ...
  FROM orders bo
  JOIN savings_calculations sc ON sc.baseline_order_id = bo.id
  JOIN order_items oi ON sc.order_item_id = oi.id
  JOIN master_products mp ON oi.master_product_id = mp.id
  WHERE bo.id = p_baseline_order_id
    AND bo.is_baseline_order = true
...
COMMENT ON FUNCTION get_savings_by_supplier IS 'Returns performance metrics for a supplier including baseline comparisons';
...
COMMENT ON FUNCTION get_baseline_order_savings IS 'Returns line-by-line savings detail for a specific baseline order (for billing/reporting)';
